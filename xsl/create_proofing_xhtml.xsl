...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  xmlns="http://www.w3.org/1999/xhtml" xmlns:hcmc="http://hcmc.uvic.ca/ns"
  xmlns:svg="http://www.w3.org/2000/svg" xmlns:saxon="http://saxon.sf.net/">
  ...
  <xsl:variable name="docsToBuild"
    select="$standaloneDocs[//catRef[starts-with(@target, 'idt:idtPrimary')]]"/>
    ...
    <html xmlns="http://www.w3.org/1999/xhtml" xml:lang="en" lang="en">
      <head>
        ...
        <style type="text/css">
          ...
          div.scene:first-of-type,
          div.scene:last-of-type{
              border-width: 0;
          }
...
<xsl:text disable-output-escaping="yes">
          ...
          <xsl:if test="$glyphsCount gt 0">
            <form>
              <p style="text-indent: -2em; margin-left: 2em;"><input type="checkbox"
                  onclick="showHideGlyphs(this)"/> Show ligatures and other special glyphs</p>
            </form>
          </xsl:if>
          <h3>Stats</h3>
          <table class="stats">
            <tr>
              ...
              <td>
                <xsl:value-of select="count(//div[@type = ('act', 'epilogue', 'prologue')])"/>
              </td>
            </tr>
            <tr>
              <td>Scenes encoded:</td>
              <td>
                <xsl:value-of select="count(//div[@type = ('scene')])"/>
              </td>
            </tr>
            <tr>
            ...
            <tr>
              <td>Prose sections encoded:</td>
              <td>
                <xsl:value-of select="count(//sp/p)"/>
              </td>
            </tr>
            <tr>
              <td>Special chars encoded:</td>
              <td>
                <xsl:value-of select="count(//g)"/>
              </td>
            </tr>
          </table>
            ...
            <table class="stats">
              ...
              <tbody>
                <xsl:for-each select="//particDesc/listPerson/person">
                  ...
                  <xsl:variable name="whoVal" select="normalize-space($thisCastItem/@xml:id)"/>
                  <xsl:variable name="speeches"
                    select="$docRoot/descendant::sp[@who = normalize-space(concat('#', $whoVal))]"/>
                  <xsl:if test="not(empty($speeches))">
                    <tr>
                      <td>
                        <xsl:value-of select="$thisCastItem/persName/reg[1]"/>
                      </td>
                      <td>
                        <xsl:value-of select="count($speeches)"/>
                      </td>
                      <td>
                        <xsl:value-of select="count($speeches/descendant::l)"/>
                      </td>
                    </tr>
                  </xsl:if>
                </xsl:for-each>
      ...
      <xsl:variable name="to"
        select="
          if (self::note) then
            ...
            @to"/>
      <xsl:variable name="hasError" select="starts-with($from, 'tln:') or starts-with($to, 'tln:')"
        as="xs:boolean"/>
          <xsl:if test="$hasError">
            <h3>ERROR ON TLN <xsl:value-of select="substring-after($from, 'tln:')"/><xsl:if
                test="$from ne $to">-<xsl:value-of select="substring-after($to, 'tln:')"
              /></xsl:if></h3>
          </xsl:if>
  ...
  <xsl:template match="note[@xml:id]/term | app/lem" mode="app">
    <xsl:param name="hasError" select="false()"/>
    <xsl:choose>
      <xsl:when test="$hasError">
        Cannot match "<span style="font-weight:bold"><xsl:apply-templates mode="#current"/></span>"
      </xsl:when>
      <xsl:otherwise>
        <h3><xsl:apply-templates mode="#current"/></h3>
      </xsl:otherwise>
    </xsl:choose>
  </xsl:template>

  <xsl:template match="note[@xml:id]/*[not(self::span) and not(self::term)] | app/*[not(self::lem)]" mode="app">
    <xsl:param name="hasError" select="false()"/>
    <xsl:if test="not($hasError)">
      <div class="{local-name()}"><em><xsl:value-of select="local-name()"/></em>
        <xsl:if test="@resp or @source">(<xsl:apply-templates select="@resp, @source"
          mode="#current"/>)</xsl:if>: <xsl:apply-templates mode="#current"/></div>
    </xsl:if>
  </xsl:template>
  ...
  <xsl:template match="note[ancestor::app]" mode="app">
    <xsl:text> [</xsl:text><xsl:apply-templates mode="#current"/>] </xsl:template>
  ...
  <xsl:template match="lb[@type = 'tln' or @subtype = 'tln']">
    ...
    <xsl:if test="$errorCount gt 0">
      ...
      <xsl:for-each select="($errorNotes, $errorColls)">
        <span class="anchor error">
          <a onclick="javascript:showRef({@xml:id},'error')">X</a>
        </span>
      </xsl:for-each>
  ...
  <xsl:template match="g[@ref]">
    ...
    <span class="g">
      <xsl:if test="$glyph">
        ...
        <xsl:if test="$glyph/mapping">
          <span class="gOld{if (contains($gId, 'igature')) then ' ligature' else ''}">
            <xsl:value-of select="$glyph/mapping[1]"/>
          </span>
        </xsl:if>
      </xsl:if>
      <span class="gModern">
  ...
  <xsl:template match="anchor[ends-with(@xml:id, '-to')]">
    ...
    <xsl:for-each select="$theseNotes">
      ...
      <span class="anchor">
        <a onclick="javascript:showRef({$thisNoteId}, 'found')">○</a>
      </span>
    </xsl:for-each>
    ...
    <xsl:for-each select="$theseColls">
      ...
      <span class="anchor">
        <a onclick="javascript:showRef({$thisCollId}, 'found')">†</a>
      </span>
    </xsl:for-each>
  ...
  <xsl:template match="@who">
    <xsl:attribute name="title"
      select="//particDesc/descendant::person[@xml:id = substring-after(., '#')]/reg"/>
  </xsl:template>
  ...
  <xsl:function name="hcmc:processAtts" as="attribute()*">
    ...
    <xsl:attribute name="class">
      <xsl:value-of select="string-join(($el/local-name(), $el/@type, $el/@rendition), ' ')"/>
    </xsl:attribute>
    <xsl:if test="$el/@style">
      <xsl:attribute name="style" select="$el/@style"/>
    </xsl:if>
